...
@RestController
@RequestMapping("/oauth/resource_body")
@Api(value = "/resource_body", tags = "Resource Body", description = "Resource Body")
	...
	public ResponseEntity<String> get(HttpServletRequest request) throws OAuthSystemException {
		...
		try {
			...
			if (TestContent.ACCESS_TOKEN_INSUFFICIENT.equals(accessToken)) {
				...
				// Return the error message
				return ResponseEntity.status(HttpStatus.FORBIDDEN).header(OAuth.HeaderType.WWW_AUTHENTICATE,
						oauthResponse.getHeader(OAuth.HeaderType.WWW_AUTHENTICATE)).build();
			}
			...
			return ResponseEntity.status(oauthResponse.getResponseStatus()).header(OAuth.HeaderType.WWW_AUTHENTICATE,
					oauthResponse.getHeader(OAuth.HeaderType.WWW_AUTHENTICATE)).build();
		}
	}
